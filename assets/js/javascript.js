function initialize() {
    ...
    google.maps.event.addListener(searchBox, 'places_changed', function() {
        ...
        if (address_components.length > 5) {
            ...
            if (address_components[len - 5].long_name != "") {
                city = address_components[len - 5].long_name;
            }
            if (address_components[len - 6].long_name != "") {
                area = address_components[len - 6].long_name;
            }
        }
        // console.log(address_components);
        // console.log("Map Address: " + places[0].formatted_address);
        // console.log("Map Name: " + places[0].name);
        // console.log(state);


        // job details
        $('.sltdcmpNme').html(places[0].name);
        $('.custmAdrs').val(places[0].formatted_address);
        let preCmpnyAdd = places[0].formatted_address
        localStorage.setItem("cmpnyAdd", preCmpnyAdd);
        $('#chngbtn').removeClass("d-none");
        $('.sltCmpnme').hide();
        $('.sltedCmpnme').show();
        $("#nwjbcmpnme").removeClass("d-none");

        $('#nwjbaddress').val(places[0].formatted_address);
        $('#nwjbcmpnme').val(places[0].name);
        $('#nwjbcmpstate').val(state);

        $('#nwjbcmpcountry').val(country);
        // $('#nwjbarea').val(area);

        $('#nwjblat').val(places[0].geometry.location.lat);
        $('#nwjblng').val(places[0].geometry.location.lng);
        ...
        $.ajax(cityNames).done(function(response) {
            ...
            for (let i = 0; i < cityy.length; i++) {
                ...
                cityArr.push(cityy[i].city);

            }

            // console.log(cityArr);
            // console.log(city);
            let citycheck = cityArr.indexOf(city);
            if (citycheck != -1) {
                $('#nwjbcmpcty').val(city);
                $('#nwjbarea').html(`<option  value="${area}">${area}</option>`);
    ...
    google.maps.event.addListener(searchBox2, 'places_changed', function() {
        ...
        $('input#intlocaddress').val(addresss);
        // $('#joblocaddress').text(addresss);
        // $('.map-choose-star').html(stars);
        // $('.locaddtxt, .jsDtPerLocAddPrev').html(addresss);
        // // $('.PsJsBdyWrp.active .JobLocationAdd').show();
        ...
        geocoder.geocode({ latLng: marker.getPosition() }, function(result, status) {
            if ('OK' === status) { // This line can also be written like if ( status == google.maps.GeocoderStatus.OK ) {
                ...
                if ($(".intlocaddtxt").text() == "") {
                    ...
                    $("#lstIntLat, #intLat").val(lat);
                    $("#lstIntLng, #intLong").val(long);
                }
                //Show The Details
                // $('.map-choose-location').text(address);
